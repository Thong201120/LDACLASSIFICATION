...
url = "https://vnexpress.net/xu-tri-va-tranh-tai-phat-dot-quy-the-nao-4383970.html"
all_contents = ''
def get_db_name(url):
...
all_urls = []
all_urls.append("https://vnexpress.net/suc-khoe/tin-tuc")
...
def extract_content(soup):
    ...
    canonicals = soup.find("ul", class_="breadcrumb").findall('a')            #lấy ra thể loại
    print(canonicals)
    canonical1 = canonicals[0]
    canonical2 = canonicals[1]
    ...
    canonical1 = replace_token(canonical1)
    canonical2 = replace_token(canonical2)
    all_contents = replace_token(all_contents)
    return (title, description, all_contents, canonical1, canonical2)
...
while link_counter < len(all_urls):
    try:
        ...
        if r.status_code == 200: #200 này là mã trạng thái HTTP, nó có nghĩa là "OK" (EG: Máy chủ đã trả lời thành công yêu cầu http).
            ...
            title, description, contents, canonical1, canonical2 = extract_content(soup) #lấy ra các nội dung cần crawl có trong html vừa chuyển đổi

            if (canonical1=='Sức khỏe'):
                print(str(link_counter) + " crawling: " + all_urls[link_counter])
                print(canonical2)
                insert_data((all_urls[link_counter], title, description, contents, canonical2)) #chèn dữ liệu vào database
        link_counter += 1
